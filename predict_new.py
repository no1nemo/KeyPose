...
def predict(model_dir, image_dir, params, camera, camera_input, mesh_file=None):
  ...
  # print(model_dir.split('/')[-4])
  model_name = model_dir.split('/')[-3]
  print(model_name)
  # exit(0)
  path = '/content/drive/MyDrive/FYP/keypose/results/{}'.format(model_name)
  curves = '/content/drive/MyDrive/FYP/keypose/results/curves'
  ...
  mae_list = []
  pose_list = [0,0,1,2,3]
  for  pose_ind in range(len(pose_list)-1):
    # print(pose_list[pose_ind])
    image_dir = image_dir.replace('pose_{}'.format(pose_list[pose_ind]), 'pose_{}'.format(pose_list[pose_ind+1]))
    ...
    for ind, fname in enumerate(filenames):
        ...
        # MAKE SURE TO CREATE THE DIRECTORY FIRST
            ...
            cv2.imwrite('/content/drive/MyDrive/FYP/keypose/results/{}/test_pose_{}_{:02d}_mae_{}.jpg'.format(model_name,pose_list[pose_ind+1],ind,round(np.mean(mae_3d)*1000)), im_large)
            key = cv2.waitKey()
            if key == ord('q'):
                break
  ...
  lt = np.array(mae_list).reshape(-1,num_kp)
  err = np.array(mae_list).reshape(-1,num_kp)#.mean(axis = -1)
  lt = tf.less(lt, 0.02)
  lt = (100.0 * tf.cast(tf.math.count_nonzero(lt, axis=[-1]), tf.float32)/tf.cast(num_kp, tf.float32)).numpy().mean()
  mae_list = np.concatenate(mae_list).flatten()
  print('MAE 3D (m): %f' %  np.mean(mae_list))
  print('MAE 3D < 2CM: %f' %  lt)
  cum_errs = []
  for i in range(0,100,1):
    lt = tf.less(err,i*0.001)
    mae_list = (100.0 * tf.cast(tf.math.count_nonzero(lt, axis=[-1]), tf.float32)/tf.cast(tf.shape(err)[-1], tf.float32)).numpy().mean()
    cum_errs.append(mae_list)
  plt.plot(cum_errs)
  plt.ylim(0,101)
  plt.plot([20,20],[0,100], 'b--', linewidth=2)
  plt.xlim(0,50)
  plt.savefig("/content/drive/MyDrive/FYP/keypose/results/{}/precision_curve.png".format(model_name))
  AUC = np.sum(np.array(cum_errs)/100)
  print('AUC: %f' % AUC )
  np.save(curves+'/precision_curve_{}_AUC_{}'.format(model_name,round(AUC)), cum_errs)
