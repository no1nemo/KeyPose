...
def generate_tfrecords(image_dirs, resize_str, params, num_samples_per_rec=20):
  ...
  for image_dir in image_dirs:
    ...
    cx = cam_orig.cx * params.camera.resx/cam_orig.resx
    cy = cam_orig.cy * params.camera.resy/cam_orig.resy
    params.camera.cx = cx
    params.camera.cy = cy
    cam = params.camera
...
def make_features(targs_pb, pf):
  ...
  for proj_targ in proj_targs:
    _, t_to_uvd, _, t_keys_uvd, _, _, _ = proj_targ
    targets_keys_uvd.append(t_keys_uvd)
    targets_to_uvd.append(t_to_uvd)
